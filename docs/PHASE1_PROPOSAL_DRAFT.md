...
3. **Associator rank allocation: beats uniform ranks, does not yet beat adaptive SVD (honest ablation).** At matched reconstruction error, the associator-guided asymmetric rank allocation outperforms uniform-rank Tucker by 1.22× on the smooth TGV, rising to 1.73× on a multi-mode perturbed TGV (Re=100, nx=64: 10% amplitude → ranks (13, 26, 26), 11.1× at 8.0e-8; 30% → (11, 22, 22), 16.4× at 5.3e-6; `scripts/run_perturbed_advantage_study.py`). However, we then ran the stronger classical baseline a reviewer would demand — standard HOSVD singular-value energy truncation (`scripts/run_svd_baseline_study.py`) — and it matches or slightly beats the associator guidance: 6–11% less memory at equal-or-lower error on the perturbed flows, with ~13× faster rank selection (19 ms vs ~250 ms at nx=64; both negligible against decomposition time). **We therefore do not claim the associator improves on standard adaptive rank selection on this benchmark.** The associator remains a novel, exact non-associative diagnostic whose unique value — if any — must be demonstrated in Phase 2 against POD and adaptive-SVD baselines on spatially heterogeneous flows, or the layer will be retired. The tensor-network compression advantage in result 2 is independent of the rank-selection method and stands either way.
